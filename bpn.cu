...
//             }
//             Net->Layer[l + 1]->Output[i] = 1 / (1 + exp(-Net->Gain * Sum));
//         }
//     }
// }
...
void BackpropagateNet(NET *Net)
{
    ...
    for (l = NUM_LAYERS - 1; l > 1; l--)
    {
        ...
        for (i = 1; i <= Net->Layer[l - 1]->Units; i++)
        {
            ...
            Net->Layer[l - 1]->Error[i] = Net->Gain * Out * (1 - Out) * Err;
        }
    }
}
...
void BackpropagateAndAdjustWeights(NET *Net) {
    INT l, i, j;
    REAL Out, Err, dWeight;
...
void SimulateNet(NET *Net, REAL *Input, REAL *Output, REAL *Target, BOOL Training, BOOL device, BOOL time)
{
    SetInput(Net, Input);
    ...
    if (!device)
    {
        startTime(&timer);
        PropagateNet(Net);
        // stopTime(&timer); printf("CPU running time: %f s\n", elapsedTime(timer));  
    }
    else
    {
        // startTime(&timer);
        total_time = 0;
        PropagateNetCUDA(Net, NUM_LAYERS);
        // stopTime(&timer); printf("GPU running time: %f s\n", elapsedTime(timer));  
    }
    ...
    if (Training)
    {
        ...
        {            
            // startTime(&timer);
            BackpropagateAndAdjustWeights(Net);
            stopTime(&timer); 
        ...
        {
            // startTime(&timer);
            BackpropagateNetCUDA(Net, NUM_LAYERS);
            AdjustWeightsCUDA(Net, NUM_LAYERS);
            // stopTime(&timer); printf("GPU running time: %f s\n", elapsedTime(timer));  
        }
    }
...
void TrainNet(NET *Net, INT Epochs, BOOL device, BOOL time)
{
    INT Year, n;
    REAL Output[M];
    float total_timer;
    ...
    for (n = 0; n < Epochs * TRAIN_YEARS; n++)
    {
        ...
        SimulateNet(Net, &(Sunspots[Year - N]), Output, &(Sunspots[Year]), TRUE, device, time);
        total_timer += total_time;
    }
    if (time && device)
        printf("GPU kernel running time: %f s\n", total_timer);  

    if (time && !device)
        printf("CPU running time: %f s\n", total_timer);  
}

void TestNet(NET *Net, BOOL device)
{
    INT Year;
    REAL Output[M];
    ...
    for (Year = TRAIN_LWB; Year <= TRAIN_UPB; Year++)
    {
        SimulateNet(Net, &(Sunspots[Year - N]), Output, &(Sunspots[Year]), FALSE, device, FALSE);
...
void EvaluateNet(NET *Net)
{
    ...
    {
        SimulateNet(Net, &(Sunspots[Year - N]), Output, &(Sunspots[Year]), FALSE, FALSE, FALSE);
        SimulateNet(Net, &(Sunspots_[Year - N]), Output_, &(Sunspots_[Year]), FALSE, FALSE, FALSE);
...
int main()
{
    ...
    BOOL GPU = FALSE;  // Change here to enable cuda code
    BOOL time = TRUE; // Change to enable timer
    ...
    startTime(&running_timer);
    do
    {
        TrainNet(&Net, 10, GPU, time);
        TestNet(&Net, GPU);
        printf("TestError: %f MinTestError: %f\n", TestError, MinTestError);
        fflush(stdout);
    ...
    stopTime(&running_timer); printf("%f s\n", elapsedTime(running_timer));
